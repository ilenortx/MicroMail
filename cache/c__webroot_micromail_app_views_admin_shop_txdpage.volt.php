...
<html>
    ...
    <body style="min-height:auto">
        <nav class="navb"><div class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>店铺管理<span class="c-gray en">&gt;</span><a href="../ShopManagement/withdrawDeposit">提现管理</a> <span class="c-gray en">&gt;</span> 提现信息<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></div></nav>
		<div class="page-container">
			...
			<form class="form form-horizontal" method="post" onsubmit="return ac_from();" enctype="multipart/form-data">
		        ...
		        <div class="row cl">
		            <label class="form-label col-xs-4 col-sm-3">店主：</label>
		            <div class="formControls col-xs-8 col-sm-3">
		            	<?= $txInfo['shop']['uname'] ?>
		            </div>
		        </div>
				...
				<?php if ($txInfo['status'] != 'S3' && $txInfo['status'] != 'S0') { ?>
		        <div class="row cl">
		            <div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
		                ...
		                <?php if ($msid) { ?><input type="hidden" name="id" id='msid' value="<?= $msid ?>"><?php } ?>
		            </div>
		        </div>
		        <?php } ?>
		    </form>
        ...
        <script>
	        $('#txbtn').click(function(){
	        	var msid = $('#msid').val();
	        	$.post('../ShopManagement/dotx', {msid:msid, type:'T0'}, function(data){
	        		var datas = jQuery.parseJSON(data);
                	if (datas.status == 1){
                        layer.msg('操作成功!', { icon: 6, time: 1000 });
                        window.location.href='../ShopManagement/withdrawDeposit';
                	}else layer.msg(datas.msg, { icon: 5, time: 1000 });
	        	});
	        });
			$('#jtxbtn').click(function(){
	        	var msid = $('#msid').val();
				$.post('../ShopManagement/dotx', {msid:msid, type:'T1'}, function(data){
					var datas = jQuery.parseJSON(data);
                	if (datas.status == 1){
                        layer.msg('操作成功!', { icon: 6, time: 1000 });
                        window.location.href='../ShopManagement/withdrawDeposit';
