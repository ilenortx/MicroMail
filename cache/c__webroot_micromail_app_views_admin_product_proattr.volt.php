    ...
    <body style="min-height:auto">
        ...
        <div class="page-container">
		    ...
		    <table id="userListDataTables" class="table table-border table-bordered table-bg table-hover table-sort">
                ...
                <tbody>
                	<?php foreach ($proAttrs as $list) { ?>
                    <tr class="text-c">
                        <td><?= $list['id'] ?></td>
		                <td><?= $list['name'] ?></td>
		                <td><?php if ($list['type'] == 'T1') { ?>所有商品<?php } else { ?>部分商品<?php } ?></td>
		                <td><?= $list['values'] ?></td>
		                <td><?= $list['sort'] ?></td>
		                <td class="td-status"><?php if ($list['status'] == 'S1') { ?><label style="color:green;">使用</label><?php } else { ?>禁用<?php } ?></td>
		                <td class="td-status">
		                	<?php if ($list['audit_result'] == 'R0') { ?><label style="color:red;">待审核</label>
		                	<?php } elseif ($list['audit_result'] == 'R1') { ?><label style="color:red;">审核中</label>
		                	<?php } elseif ($list['audit_result'] == 'R2') { ?><label style="color:red;">审核中</label>
		                	<?php } elseif ($list['audit_result'] == 'R3') { ?><label style="color:green;">通过</label>
		                	<?php } ?></td>
                        <td class="td-manage">
