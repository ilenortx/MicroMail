        	...
        	<form class="form form-horizontal">
				...
				<div class="row cl">
					...
					<div class="formControls col-xs-8 col-sm-9">
		            	...
		            	name="stime" id="stime" value="<?= $cpInfo['stime'] ?>" style="width:160px;" <?php if ($type == 'edit') { ?>disabled="disabled" readonly="readonly"<?php } ?>>
		            	至
		            	<input type="text" onfocus="WdatePicker({ dateFmt: 'yyyy-MM-dd HH:mm',minDate:'#F{$dp.$D(\'stime\')}' })" placeholder="失效时间"
		            	id="etime" name="etime" value="<?= $cpInfo['etime'] ?>" class="input-text Wdate" style="width:160px;" <?php if ($cpInfo['status'] == 'S3') { ?>disabled="disabled" readonly="readonly"<?php } ?>>
					</div>
				</div>
				<div class="row cl">
					...
					<div class="formControls col-xs-8 col-sm-9">
						<div class="radio-box">
						    <input type="radio" class="cp_type" name="cp_type" value="1" <?php if (!$cpInfo['cptype'] || $cpInfo['cptype'] == '1') { ?> checked="checked" <?php } ?>>
						    <label for="radio-1">砍价到底才可购买</label>
						</div>
						<div class="radio-box" style="margin-left:20px">
						    <input type="radio" class="cp_type" value="2" name="cp_type" <?php if ($cpInfo['cptype'] == '2') { ?> checked="checked" <?php } ?>>
	        ...
	        function getPros(proIds){
	        	...
	        	$.post('../CutPriceSprites/getPros', {proIds:proIds}, function(data){
	        		...
	        		if (datas.status == 1){
	        			...
	        			<?php if ($type == 'add') { ?>phtml = '<div class="jz_upload pd_choose jz_upload_normal" onclick="win_open(\'../CutPriceSprites/chooseProPage?proIds='+proIds+'\',670,580)"><div class="jz_upload_icon"></div></div>';<?php } ?>
