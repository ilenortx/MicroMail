...
<script>
	...
	function window_opener() {
	    window.opener.getPros(choosed);
	
	    window.close();
	}
	 
	function choosePor(obj){
		var proInfo = $('.pro-info');
		for (var i=0; i<proInfo.length; ++i){
			$(proInfo[i]).find('img').css({'border-color':'rgb(170, 170, 170)'});
			$(proInfo[i]).find('.chooseTopFlag').css({'display':'none'});
			$().css({'border-color':'rgb(170, 170, 170)'});
		}
