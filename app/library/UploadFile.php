 ...
 */
class UploadFile{
	private $max_size   	= '20000000'; //设置上传文件的大小，此为20M
	...
	public function upload_file($file){
		...
		if($f = move_uploaded_file($this->file_tmp_name, $this->uploaded)){
			$this->msg = array('savepath'=>$this->save_path.'/', 'savename'=>$this->new_name);
			return true;
		}else{
			$this->msg = '文件上传失败';
			return false;
	...
	public function set_file_name(){
		if($this->rand_name==true){
			$a = explode(' ', microtime());
			$t = $a[1].($a[0]*1000000).rand(100, 999);
