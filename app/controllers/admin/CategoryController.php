...
class CategoryController extends AdminBase{
    ...
    public function cgAPageAction(){
        ...
        $this->view->allParm = ProductParm::find('disabled=0')->toArray();
    ...
    public function cgInfoAction($cgid){
    	...
    	$cgiArr = array(
    			'id'=>'','tid'=>'','name'=>'', 'bz_1'=>'','concent'=>'','sort'=>'','parm_id'=>0,
    	);
    ...
    private function getAllParm(){
        $parm_data = ProductParm::find('disabled=0');
        return $parm_data->toArray();
    }
    ...
    public function saveAction(){
    	if ($this->request->isPost()){
    		...
    		$cg->sort = $_POST['sort'];
            if(isset($cg->parm_id)){
                if($cg->parm_id != $_POST['parm_id']){
                    $product_res = $cg->Product;
