...
var alreadySetSkuPrice = 0;
var alreadySetSkuStock = 0;

$(function(){
	//sku属性发生改变时,进行表格创建
	$(document).on("change",'.sku_value',function(){
		alreadySetSkuPrice = $('#price_yh').val();
		if (alreadySetSkuPrice) createSkuTable();
		else alert("请先正确填写价格!");
	});
});
...
//			});
			...
			//var alreadySetSkuPrice = alreadySetSkuPrice;//已经设置的SKU价格
...
function getAlreadySetSkuVals(){
	if (!alreadySetSkuVals) alreadySetSkuVals = {};
	//获取设置的SKU属性值
 ...
 * 设定已经设置的SKU值 
 */
function setAlreadySetSkuVals(skus){
	...
	if (keys.length) alreadySetSkuVals = skus;
}
/**
 * 获取设定价格
 */
function getSetPrice(obj){
	alreadySetSkuPrice = $(obj).val();
}
...
function getSkuVals(){
	...
	var assvj = {};
	for(var k in alreadySetSkuVals){
		if (k){
			if (!alreadySetSkuVals[k].skuPrice || !alreadySetSkuVals[k].skuStock) {
				alert("请正确填写SUK数据！");
				return false;
			}
			assvj[k] = alreadySetSkuVals[k];
		}
	}
...
function getProAttrs(){
	...
	$("input[type='checkbox'][class*='sku_value']").each(function(){
		if($(this).is(":checked")){
			skuValueId = $(this).attr("propvalid");//SKU值主键
			skuPropId = $(this).attr("propid");//SKU类型主键
			if (!proAttr[skuPropId]) proAttr[skuPropId] = '';
			proAttr[skuPropId] += skuValueId+',';
		}
	});
